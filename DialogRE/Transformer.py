...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, inputs, mask=None, key_padding_mask=None, structure=None):
        ...
        src = inputs
        src2, _ = self.self_attn(src, src, src, mask=mask, key_padding_mask=key_padding_mask, structure=structure, )
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        tgt = self.norm1(tgt)
        tgt2 = self.cross_attn(tgt, memory, memory, mask=memory_mask, key_padding_mask=memory_key_padding_mask)[0]
...
class DoubleAttnTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, d_enc, heads, d_con=512, d_ff=2048, dropout=0.1, att_drop=0.1, activation="relu",
                 dual_enc=True):
        ...
        self.sent_cross_attn = MultiHeadedAttention(
            heads, d_enc, query_dim=d_model, dropout=att_drop, use_structure=False
        )
        if self.d_enc != d_model and not dual_enc:
            self.kv_map = nn.Linear(self.d_enc, d_model)
        else:
            self.kv_map = None
        ...
        if dual_enc:
            ...
            self.fuse_linear = nn.Linear(self.d_enc + self.d_con, d_model)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, d_ff)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(d_ff, d_model)
    ...
    def forward(
        ...
        if self.dual_enc:
            graph_tgt2 = self.graph_cross_attn(
                tgt, graph_memory, graph_memory, mask=graph_memory_mask,
                key_padding_mask=graph_memory_key_padding_mask)[0]
