...
if __name__ == "__main__":
    ...
    s_time = time.time()
    train_set = AMRDialogDataSet(FLAGS.train_path, tokenize_fn, word_rel_vocab=relation2id, word_vocab=word2id,
                                 concept_vocab=con2id, path_vocab=path2id, save_path=FLAGS.save_data + '/train',
                                 data_type='std')
    print("Loading trainset takes {:.3f}s".format(time.time() - s_time))

    print("Loading dev data ...")
    s_time = time.time()
    dev_set = AMRDialogDataSet(FLAGS.dev_path, tokenize_fn, word_rel_vocab=relation2id, word_vocab=word2id,
                               concept_vocab=con2id, path_vocab=path2id, save_path=FLAGS.save_data + '/dev',
                               data_type='std')
    print("Loading devset takes {:.3f}s".format(time.time() - s_time))

    print("Loading test data ...")
    s_time = time.time()
    test_set = AMRDialogDataSet(FLAGS.test_path, tokenize_fn, word_rel_vocab=relation2id, word_vocab=word2id,
                                concept_vocab=con2id, path_vocab=path2id, save_path=FLAGS.save_data + '/test',
                                data_type='std')
