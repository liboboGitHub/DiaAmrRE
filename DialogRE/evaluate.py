...
def getpredict(result, T1=0.5, T2=0.4):
    for i in range(len(result)):
        ...
        for j in range(len(result[i])):
            if result[i][j] > T1:       # score > 0.5
                r += [j]
            if result[i][j] > maxl:     # record maxl
                maxl = result[i][j]
                maxj = j
        if len(r) == 0:                 # maxl <= 0.5
            if maxl <= T2:              # maxl < 0.4
...
if __name__ == "__main__":
    ...
    parser.add_argument("--devdata", default=None, type=str, required=True, help="dev data")
    parser.add_argument("--testdata", default=None, type=str, required=True, help="test data")
    parser.add_argument("--f1dev", default=None, type=str, required=True, help="Dev logits (f1).")
    parser.add_argument("--f1test", default=None, type=str, required=True, help="Test logits (f1).")
    parser.add_argument("--f1cdev", default=None, type=str, required=True, help="Dev logits (f1c).")
    parser.add_argument("--f1ctest", default=None, type=str, required=True, help="Test logits (f1c).")
    ...
    with open(devf, "r", encoding="utf8") as f:
        datadev = json.load(f)
    with open(testf, "r", encoding="utf8") as f:
    ...
    bestT2 = bestf_1 = 0
    for T2 in range(51):            # test different threshold
        dev = getresult(f1dev)
        devp = getpredict(dev, T2=T2 / 100.0)
        precision, recall, f_1 = evaluate(devp, datadev)
        if f_1 > bestf_1:           # record best T2
            bestf_1 = f_1
            bestT2 = T2 / 100.0
    ...
    with open('predict-dev-std.json', 'w', encoding='utf-8') as fout:
        json.dump(devp, fout, indent=4)
    ...
    with open('predict-test-std.json', 'w', encoding='utf-8') as fout:
        json.dump(testp, fout, indent=4)

    dev = getresult(f1cdev)
    devp = getpredict(dev, T2=bestT2)
    precision, recall, f_1c = evaluate_f1c(devp, datadev)
    print("dev (P_c R_c F1_c)", precision, recall, f_1c)
    with open('predict-dev-c.json', 'w', encoding='utf-8') as fout:
        json.dump(devp, fout, indent=4)
    ...
    with open('predict-test-c.json', 'w', encoding='utf-8') as fout:
        json.dump(testp, fout, indent=4)
